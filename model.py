...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv10 = nn.Sequential(nn.Conv3d(1,16,3,1,1),nn.BatchNorm3d(16),
        nn.ReLU())
...
if __name__ == '__main__':
    net = Model()        
    x = torch.randn(1,1,176,176,160, dtype=torch.float)    
    print('input:',  x.size())
    with torch.no_grad():
        y = net(x)
    print('output:', y.size())
    prm = net.getprm(net)
    print('Model Parameters (M) :', np.round(prm/1e6,2)) 
    size = (prm * 32)/(8 * 1024 * 1024)
    print('Model Size (MB) :', np.round(size,2))       
